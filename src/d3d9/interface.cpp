...
d3d9_proxy::d3d9_proxy(IDirect3D9* orig): m_d3d(orig)
{
}
...
HRESULT __stdcall d3d9_proxy::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
{
	...
	printf("CreateDevice hr = %x\n", hr);
	if (!SUCCEEDED(hr)) return hr;

	*ppReturnedDeviceInterface = new d3d9_device_proxy(reinterpret_cast<IDirect3DDevice9Ex*>(device), pPresentationParameters, hFocusWindow);
	return hr;
}
