...
std::function<void(d3d9_device_proxy*)> on_created_device = [](auto){};

d3d9_device_proxy::d3d9_device_proxy(IDirect3DDevice9* orig, D3DPRESENT_PARAMETERS* pp, HWND window): m_hwnd(window), m_device(orig)
{
	memcpy(&m_pp, pp, sizeof(D3DPRESENT_PARAMETERS));
	m_present_cb = [](auto) {};

	on_created_device(this);
	d3d9_device_proxy::_instance = this;
}
...
ULONG __stdcall d3d9_device_proxy::Release(void)
{
	...
	if (!count) 
	{
		d3d9_device_proxy::_instance = nullptr;
		delete this;
	}
...
d3d9_device_proxy* d3d9_device_proxy::_instance = nullptr;
d3d9_device_proxy* d3d9_device_proxy::instance()
{
	return d3d9_device_proxy::_instance;
}

void d3d9_device_proxy::on_create(std::function<void(d3d9_device_proxy*)> cb)
{
	on_created_device = cb;
}

void d3d9_device_proxy::on_present(std::function<void(d3d9_device_proxy*)> callback)
{
	m_present_cb = callback;
}

HWND d3d9_device_proxy::get_hwnd() const
{
	return m_hwnd;
}
